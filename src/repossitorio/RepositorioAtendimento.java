...
public class RepositorioAtendimento extends Repositorio<Atendimento> {
    ...
    @Override
    public Atendimento localizar(Object chave) {
        try {
            int id = (int) chave;
            return Util.getManager().find(Atendimento.class, id);
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public List<Atendimento> listar() {
        TypedQuery<Atendimento> q = Util.getManager().createQuery("SELECT a FROM Atendimento a ORDER BY a.id", Atendimento.class);
        return q.getResultList();
    }

    // Consulta 1: Atendimentos na data X
    public List<Atendimento> buscarPorData(LocalDate data) {
        TypedQuery<Atendimento> q = Util.getManager().createQuery(
